import { AucEthChainParams } from './ethereum';

export interface AucNetworkOption {
  /** Currency icon url */
  icon: string;
  name: string;
  chainId: string;
  /** Currency symbol. You can use AucNativeCurrencies[chainID].name or other string. */
  symbol: string;

  /** Ex: https://etherscan.io. <br>
   * You can use {@link AucBlockExplorerUrls[chainId][0]} or set other url.
   * */
  blockExplorerUrl: string;

  /** Ex: https://api.etherscan.io/api. <br>
   * Used for getting transactions information.
   * You can use {@link AucBlockExplorerUrls[chainId]} or set other url.
   * */
  blockExplorerApiUrl?: string;

  /** Sets option as active */
  isActive?: boolean;
  disabled?: boolean;

  /** This params used for adding new network to the Metamask. */
  chainParams?: AucEthChainParams;
}
