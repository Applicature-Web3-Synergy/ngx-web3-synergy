...
export class ApplicatureCustomizeDialogDirective implements OnInit {
  @Input() config?: ApplicatureCustomizeDialogConfig;
  ...
  public mapConfig(): void {
    ...
    Object.keys(this.config).forEach((key: string) => {
      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.CLASSES) {
        this.setClasses([].concat(this.config[key]));

        return;
      }

      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.WIDTH) {
        this.setStyle('width', this.config[key]);

        return;
      }

      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.HEIGHT) {
        this.setStyle('height', this.config[key]);

        return;
      }

      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.MIN_WIDTH) {
        this.setStyle('min-width', this.config[key]);

        return;
      }

      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.MIN_HEIGHT) {
        this.setStyle('min-height', this.config[key]);

        return;
      }

      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.MAX_WIDTH) {
        this.setStyle('max-width', this.config[key]);

        return;
      }

      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.MAX_HEIGHT) {
        this.setStyle('max-height', this.config[key]);

        return;
      }

      if (key === APPLICATURE_CUSTOMIZE_DIALOG_CONFIG_KEYS.POSITION) {
        this.setClasses([].concat(this.config[key]));
